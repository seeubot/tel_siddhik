...
try {
  ...
  console.error('❌ Firebase Admin init failed — check FIREBASE_SERVICE_ACCOUNT or serviceAccountKey.json:', err.message);
  process.exit(1);
}
// ─────────────────────────────────────────────────────────────────────────────
...
const BanSchema = new mongoose.Schema({
  // Can be a deviceId OR a Firebase UID — stored in the same collection, source field distinguishes
  deviceId:       { type: String, required: true, unique: true, index: true },
  firebaseUid:    { type: String, default: null, index: true }, // ← NEW: UID-level ban
  reason:         String,
...
const ReportSchema = new mongoose.Schema({
  ...
  reporterUid:      String, // ← NEW
  reportedDeviceId: { type: String, index: true },
  reportedUid:      { type: String, index: true }, // ← NEW
...
const ReportCountSchema = new mongoose.Schema({
  deviceId:   { type: String, required: true, unique: true, index: true },
  firebaseUid:{ type: String, default: null, index: true }, // ← NEW
...
// ── NEW: Permanent user account (replaces ephemeral OreyId) ──────────────────
const UserAccountSchema = new mongoose.Schema({
  firebaseUid:  { type: String, required: true, unique: true, index: true },
  oreyId:       { type: String, required: true, unique: true, index: true }, // e.g. OREY-AB3KP
...
// ── LEGACY: keep OreyId model for backward-compat during migration ────────────
...
// In-memory maps
const oreyIds              = new Map(); // legacy ephemeral IDs
const rooms                = new Map();
const randomQueue          = [];
...
// uid → socketId  (for online presence look-up)
const uidToSocket          = new Map(); // ← NEW

let bannedDevicesCache     = new Map();
let bannedUidsCache        = new Set(); // ← NEW: UID-level bans
let appConfig              = null;
...
async function initDB() {
  ...
  if (!cfg) {
    cfg = {
      _id: 'main',
      android: { versionCode: 1, versionName: '1.0.0', updateType: 'flexible', updatePriority: 2, updateMessage: 'New features available!', updateTitle: 'Update Available', whatsNew: ['HD Video', 'Push notifications', 'Connection stability', 'Safety features'], forceUpdateMinVersion: 1, downloadUrl: 'https://play.google.com/store/apps/details?id=com.orey.app', changelog: 'https://orey.app/changelog' },
  ...
  for (const b of bans) {
    bannedDevicesCache.set(b.deviceId, b);
    if (b.firebaseUid) bannedUidsCache.add(b.firebaseUid);
  }
...
/**
 * Generate a unique permanent Orey display ID.
 * Format: OREY-XXXXX  (5 chars from an unambiguous charset)
 */
...
async function generateUniqueOreyId() {
  let displayId, attempts = 0;
  do {
    displayId = generateOreyDisplayId();
    attempts++;
    if (attempts > 50) throw new Error('Could not generate unique Orey ID');
  } while (await UserAccount.exists({ oreyId: displayId }));
  return displayId;
}
...
function isUidBanned(uid) {
  // ← NEW: check UID-level ban (permanent account ban)
  return bannedUidsCache.has(uid);
}

async function banDeviceAndDisconnect(deviceId, banInfo) {
  bannedDevicesCache.set(deviceId, banInfo);
  if (banInfo.firebaseUid) bannedUidsCache.add(banInfo.firebaseUid);
  await Ban.findOneAndUpdate({ deviceId }, { $set: banInfo }, { upsert: true }).catch(() => {});

  const socketsToDisconnect = [];
  for (const [, socket] of io.sockets.sockets) {
    if (socket.data.deviceId === deviceId || (banInfo.firebaseUid && socket.data.firebaseUid === banInfo.firebaseUid)) {
...
function _createRoom(selfSocket, partnerSocket) {
  ...
  const selfData = {
    ...
    uid:      selfSocket.data.firebaseUid || null,
  };
  const partnerData = {
    ...
    uid:      partnerSocket.data.firebaseUid || null,
  };
  ...
  console.log(`🤝 Room: ${roomId} | ${selfSocket.id.slice(0, 8)} ↔ ${partnerSocket.id.slice(0, 8)}`);
}
...
function attemptMatch(newSocketId) {
  ...
  if (gender && genderMatcher.isQueued(newSocketId)) {
    ...
    if (!genderMatcher.isTimerExpired(newSocketId)) return;
  }
...
const publicPath = (() => {
  const paths = [path.join(__dirname, 'public'), path.join(__dirname, '..', 'public'), path.join(__dirname, 'src', 'public')];
  for (const p of paths) { if (fs.existsSync(p)) { console.log('📂 Public:', p); return p; } }
...
// Gender timer expiry → random queue
genderMatcher.onTimerExpire((socketId, gender) => {
  ...
  if (!randomQueue.includes(socketId)) randomQueue.push(socketId);
...
app.get('/health', (_req, res) => res.json({ status: 'ok', timestamp: new Date().toISOString(), uptime: process.uptime(), activeConnections: io.engine.clientsCount, bannedDevices: bannedDevicesCache.size, bannedUids: bannedUidsCache.size, activeAdminSessions: ADMIN_SESSIONS.size, memory: +((process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)), dbState: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected' }));

// ── NEW: Firebase auth endpoint (REST — for pre-socket verification if needed) ─
app.post('/api/auth/firebase', verifyApiKey, async (req, res) => {
  ...
  // Check UID-level ban
  if (isUidBanned(uid)) {
    const ban = [...bannedDevicesCache.values()].find(b => b.firebaseUid === uid);
    return res.status(403).json({ error: 'Account banned', banned: true, reason: ban?.reason, permanent: ban?.permanent });
  }
  if (deviceId && isDeviceBanned(deviceId)) {
    const ban = isDeviceBanned(deviceId);
    return res.status(403).json({ error: 'Device banned', banned: true, reason: ban.reason, expiresAt: ban.expiresAt });
  }

  // Find or create permanent account
  let account = await UserAccount.findOne({ firebaseUid: uid });
  if (!account) {
    const oreyId = await generateUniqueOreyId();
    ...
    console.log(`🆕 New account: ${uid.slice(0, 8)} → ${oreyId}`);
  } else {
    // Update last seen & profile data
    account.lastSeen    = new Date();
    account.email       = decoded.email   || account.email;
    account.displayName = decoded.name    || account.displayName;
    account.photoURL    = decoded.picture || account.photoURL;
    await account.save();
  }
...
app.get('/api/version', (req, res) => {
  ...
  res.json(cv < pc.versionCode ? { updateAvailable: true, ...pc } : { updateAvailable: false, currentVersion: pc.versionName });
});
...
app.get('/api/notifications', (req, res) => {
  ...
  let filtered = notificationsCache.filter(n => {
    if (n.expiresIn && n.expiresIn > 0) { const ed = new Date(n.timestamp); ed.setDate(ed.getDate() + n.expiresIn); if (new Date() > ed) return false; }
    return n.id > lastId;
  });
...
app.get('/api/config', (req, res) => {
  const cv = parseInt(req.query.version) || 0;
  res.json({ features: { videoCall: true, hdVideo: cv >= 2, groupCall: false, screenShare: cv >= 3, reporting: true, safetyFeatures: true, chat: true, genderMatching: true, selfBlur: true }, videoQuality: appConfig.videoQuality, iceServers: ICE_SERVERS, safety: appConfig.safety, reportReasons: { highPriority: HIGH_PRIORITY_REASONS, regular: REGULAR_REASONS }, urls: { mainApp: process.env.MAIN_APP_URL || 'https://parallel-elsi-seeutech-50a3ab2e.koyeb.app/', help: 'https://orey.app/help', safety: 'https://orey.app/safety', guidelines: 'https://orey.app/community-guidelines' }, maintenance: appConfig.maintenance });
});
...
app.post('/api/device/check-ban', verifyApiKey, (req, res) => {
  ...
  if (!deviceId && !firebaseUid) return res.status(400).json({ error: 'deviceId or firebaseUid required' });
  if (firebaseUid && isUidBanned(firebaseUid)) {
    const ban = [...bannedDevicesCache.values()].find(b => b.firebaseUid === firebaseUid);
    return res.status(403).json({ banned: true, reason: ban?.reason, permanent: ban?.permanent });
  }
  if (deviceId) { const bi = isDeviceBanned(deviceId); if (bi) return res.status(403).json({ banned: true, ...bi }); }
  res.json({ banned: false });
});

app.post('/api/report', verifyApiKey, async (req, res) => {
  const { reporterDeviceId, reporterUid, reportedDeviceId, reportedUid, reportedUserId, reason, description, evidence } = req.body;
  ...
  await Report.create({ id: reportId, reporterDeviceId, reporterUid: reporterUid || null, reportedDeviceId, reportedUid: reportedUid || null, reportedUserId: reportedUserId || null, reason, description: description || '', evidence: evidence || null, timestamp: new Date().toISOString(), status: 'pending', autoBanned: false, isHighPriority: isHP });
  const counter = await ReportCount.findOneAndUpdate({ deviceId: reportedDeviceId }, { $inc: { total: 1, ...(isHP ? { highPriority: 1 } : {}) }, $set: { firebaseUid: reportedUid || null }, $addToSet: { reportedBy: reporterDeviceId } }, { upsert: true, new: true });
  ...
  if ((isHP && counter.highPriority >= HIGH_PRIORITY_BAN_THRESHOLD) || (!isHP && counter.total >= AUTO_BAN_THRESHOLD)) {
    if (appConfig.safety.autoBanEnabled) {
      ...
      bi = { deviceId: reportedDeviceId, firebaseUid: reportedUid || null, reason: isHP ? `Auto-banned (HP): ${counter.highPriority} reports` : `Auto-banned: ${counter.total} reports`, timestamp: new Date().toISOString(), expiresAt: bd > 0 ? new Date(Date.now() + bd * 3600000) : null, durationHours: bd > 0 ? bd : null, permanent: bd === 0, source: 'auto', isHighPriority: isHP, reportIds: [] };
  ...
  res.json({ success: true, reportId, reportCount: counter.total, highPriorityCount: counter.highPriority, isHighPriority: isHP, autoBanned: ab, banInfo: bi || null });
});
...
app.post('/admin/login', (req, res) => {
  ...
  const st = generateSessionToken(), ea = Date.now() + SESSION_DURATION_MS;
  ADMIN_SESSIONS.set(st, { expiresAt: ea, createdAt: Date.now() });
  console.log('🔐 Admin login');
  res.json({ success: true, sessionToken: st, expiresAt: new Date(ea).toISOString() });
});
...
app.get('/admin/notifications', verifySession, (_req, res) => {
...
app.post('/admin/reports/:reportId/action', verifySession, async (req, res) => {
  ...
  switch (action) {
    case 'ban': {
      ...
      const bi = { deviceId: report.reportedDeviceId, firebaseUid: report.reportedUid || null, reason: report.reason, timestamp: new Date().toISOString(), expiresAt: d > 0 ? new Date(Date.now() + d * 3600000) : null, durationHours: d > 0 ? d : null, permanent: d === 0, source: 'admin', isHighPriority: report.isHighPriority, reportIds: [reportId] };
      await banDeviceAndDisconnect(report.reportedDeviceId, bi); report.status = 'banned'; break;
    }
    case 'dismiss': report.status = 'dismissed'; break;
    case 'warn':
      report.status = 'warned';
      for (const [, s] of io.sockets.sockets) { if (s.data.deviceId === report.reportedDeviceId || (report.reportedUid && s.data.firebaseUid === report.reportedUid)) s.emit('warning', { reason: report.reason, message: 'Warning for violating community guidelines.' }); }
      break;
...
app.post('/admin/ban-device', verifySession, async (req, res) => {
  const { deviceId, firebaseUid, reason, durationHours } = req.body;
  if (!deviceId) return res.status(400).json({ error: 'deviceId required' });
  if (isDeviceBanned(deviceId)) return res.status(400).json({ error: 'Already banned' });
  const bi = { deviceId, firebaseUid: firebaseUid || null, reason: reason || 'Violation of terms', timestamp: new Date().toISOString(), expiresAt: durationHours ? new Date(Date.now() + durationHours * 3600000) : null, durationHours: durationHours || null, permanent: !durationHours, source: 'manual', isHighPriority: false, reportIds: [] };
...
app.delete('/admin/ban-device/:deviceId', verifySession, async (req, res) => {
  ...
  const ban = bannedDevicesCache.get(found);
  if (ban?.firebaseUid) bannedUidsCache.delete(ban.firebaseUid);
...
app.get('/admin/banned-devices', verifySession, async (req, res) => {
  for (const [deviceId, info] of bannedDevicesCache.entries()) {
    if (info.expiresAt && Date.now() > new Date(info.expiresAt).getTime()) {
      if (info.firebaseUid) bannedUidsCache.delete(info.firebaseUid);
      bannedDevicesCache.delete(deviceId);
      await Ban.deleteOne({ deviceId }).catch(() => {});
    }
  }
  const list = [...bannedDevicesCache.entries()].map(([deviceId, info]) => ({ deviceId, firebaseUid: info.firebaseUid || null, reason: info.reason, timestamp: info.timestamp, expiresAt: info.expiresAt || null, permanent: info.permanent || false, isHighPriority: info.isHighPriority || false, source: info.source || 'manual' })).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  res.json({ bannedDevices: list, total: list.length });
});

// ── NEW: Admin — look up account by UID ──────────────────────────────────────
app.get('/admin/user/:uid', verifySession, async (req, res) => {
  const account = await UserAccount.findOne({ firebaseUid: req.params.uid }).lean();
  if (!account) return res.status(404).json({ error: 'User not found' });
  const reports = await Report.find({ reportedUid: req.params.uid }).sort({ timestamp: -1 }).lean();
  res.json({ account, banned: isUidBanned(req.params.uid), reports, reportCount: reports.length });
});

app.get('/admin/stats', verifySession, async (req, res) => {
  res.json({ activeConnections: io.engine.clientsCount, activeRooms: rooms.size, bannedDevices: bannedDevicesCache.size, bannedUids: bannedUidsCache.size, pendingReports: await Report.countDocuments({ status: 'pending' }), totalNotifications: notificationsCache.length, activeAdminSessions: ADMIN_SESSIONS.size, uptime: process.uptime(), memoryMB: +((process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)), safetySettings: appConfig.safety, dbState: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', genderStats: { ...genderMatcher.stats(), randomQueueLength: randomQueue.length } });
});
...
io.on('connection', (socket) => {
  ...
  // ── NEW: Firebase auth over socket ──────────────────────────────────────────
  // Client sends this immediately after connecting.
  // Payload: { idToken: string, deviceId?: string }
  socket.on('firebase-auth', async ({ idToken, deviceId }) => {
    if (!idToken) { socket.emit('auth-error', { error: 'idToken required' }); return; }

    let decoded;
    try {
      decoded = await admin.auth().verifyIdToken(idToken);
    } catch (err) {
      socket.emit('auth-error', { error: 'Invalid Firebase token' });
      return;
    }
    ...
    if (isUidBanned(uid)) {
      const ban = [...bannedDevicesCache.values()].find(b => b.firebaseUid === uid);
      socket.emit('device-banned', { reason: ban?.reason, permanent: ban?.permanent });
      socket.disconnect(true);
      return;
    }
    if (deviceId && isDeviceBanned(deviceId)) {
      socket.emit('device-banned', isDeviceBanned(deviceId));
      socket.disconnect(true);
      return;
    }
    ...
    // Attach to socket
    socket.data.firebaseUid = uid;
    socket.data.oreyId      = account.oreyId;
    socket.data.userName    = account.displayName || 'Anonymous';
    ...
    socket.emit('auth-success', {
      ...
      email:       account.email,
      photoURL:    account.photoURL,
      permanent:   true, // ← signals to client that this ID is forever
    });

    console.log(`✅ Auth: ${uid.slice(0, 8)} | Orey: ${account.oreyId}`);
  });
  // ───────────────────────────────────────────────────────────────────────────
  ...
  socket.on('report-user', async ({ reportedDeviceId, reportedUserId, reportedUid, reason, description }) => {
    ...
    const reportId = generateRoomId(), isHP = HIGH_PRIORITY_REASONS.includes(reason);
    await Report.create({ id: reportId, reporterDeviceId, reporterUid: socket.data.firebaseUid || null, reportedDeviceId, reportedUid: reportedUid || null, reportedUserId: reportedUserId || null, reason, description: description || '', evidence: null, timestamp: new Date().toISOString(), status: 'pending', autoBanned: false, isHighPriority: isHP });
    ...
    if ((isHP && counter.highPriority >= HIGH_PRIORITY_BAN_THRESHOLD) || (!isHP && counter.total >= AUTO_BAN_THRESHOLD)) {
      if (appConfig.safety.autoBanEnabled) {
        const bd = isHP ? 0 : DEFAULT_BAN_DURATION_HOURS;
        const bi = { deviceId: reportedDeviceId, firebaseUid: reportedUid || null, reason: `Auto-banned: ${counter.total} reports`, timestamp: new Date().toISOString(), expiresAt: bd > 0 ? new Date(Date.now() + bd * 3600000) : null, durationHours: bd || null, permanent: bd === 0, source: 'auto', isHighPriority: isHP, reportIds: [] };
        await Report.updateMany({ reportedDeviceId }, { $set: { status: 'auto_banned', autoBanned: true } });
        await banDeviceAndDisconnect(reportedDeviceId, bi);
        ab = true;
      }
  ...
  // Legacy ephemeral Orey ID registration (kept for backward compat)
  socket.on('register-orey-id', ({ oreyId, userName }) => {
    // If user is already Firebase-authed, skip legacy flow
    if (socket.data.firebaseUid) {
      socket.emit('orey-id-registered', { oreyId: socket.data.oreyId, expiresAt: null, permanent: true });
      return;
    }
    ...
    socket.emit('orey-id-registered', { oreyId: vid, expiresAt: entry.expiresAt, permanent: false });
  });

  socket.on('connect-by-orey-id', async ({ targetOreyId }) => {
    const vid = validateOreyId(targetOreyId);
    if (!vid) { socket.emit('orey-id-invalid', { error: 'Invalid format' }); return; }

    // First try permanent accounts
    const targetAccount = await UserAccount.findOne({ oreyId: vid }).lean();
    if (targetAccount) {
      const targetSocketId = uidToSocket.get(targetAccount.firebaseUid);
      const ts = targetSocketId ? io.sockets.sockets.get(targetSocketId) : null;
      if (!ts) { socket.emit('orey-id-offline'); return; }
      ...
      const calleeD = { userName: targetAccount.displayName || 'Anonymous', oreyId: vid, deviceId: ts.data.deviceId || null, gender: ts.data.gender || null };
      rooms.get(roomId).set(socket.id, cd); rooms.get(roomId).set(ts.id, calleeD);
      const rd = { roomId, videoQuality: appConfig.videoQuality, iceServers: ICE_SERVERS };
      socket.emit('room-joined', { ...rd, peers: [{ socketId: ts.id, ...calleeD }] });
      ts.emit('room-joined', { ...rd, peers: [{ socketId: socket.id, ...cd }] });
      ts.emit('incoming-call', { fromName: cd.userName, fromOreyId: cd.oreyId });
      return;
    }

    // Fallback: legacy ephemeral
    let entry = oreyIds.get(vid);
    if (!entry) { for (const [k, v] of oreyIds.entries()) { if (k.startsWith(targetOreyId)) { entry = v; break; } } }
    ...
    const rd2 = { roomId, videoQuality: appConfig.videoQuality, iceServers: ICE_SERVERS };
    socket.emit('room-joined', { ...rd2, peers: [{ socketId: entry.socketId, ...calleeD }] });
    ts.emit('room-joined', { ...rd2, peers: [{ socketId: socket.id, ...cd }] });
    ts.emit('incoming-call', { fromName: cd.userName, fromOreyId: cd.oreyId });
  });
  ...
  socket.on('set-gender', ({ gender }) => {
    ...
    socket.emit('gender-set', { gender: normalised, accepted: normalised !== null, message: normalised ? `Matching you with ${normalised === 'male' ? 'females' : 'males'} first.` : 'Gender cleared — standard random matching.' });
  ...
  // ── NEW: Self-blur feature ──────────────────────────────────────────────────
  // Client emits this when the user taps the "Blur my preview" button.
  // The server stores the state and forwards it to the partner so they know
  // the other person has blurred their camera (UI badge / indicator).
  socket.on('toggle-self-blur', ({ roomId, enabled }) => {
    if (!roomId) return;
    const room = rooms.get(roomId);
    if (!room || !room.has(socket.id)) return;

    // Persist blur state on the socket for reconnection awareness
    socket.data.selfBlurEnabled = !!enabled;

    // Notify the partner
    socket.to(roomId).emit('peer-blur-changed', {
      socketId:    socket.id,
      blurEnabled: !!enabled,
      // Optional: give the partner something to show in their UI
      message:     enabled
        ? `${socket.data.userName || 'Your partner'} blurred their camera.`
        : `${socket.data.userName || 'Your partner'} unblurred their camera.`,
    });

    // Acknowledge back to the sender
    socket.emit('self-blur-ack', { enabled: !!enabled });
    console.log(`🌫️  Blur: ${socket.id.slice(0, 8)} → ${enabled ? 'ON' : 'OFF'} in room ${roomId}`);
  });
  // ───────────────────────────────────────────────────────────────────────────
  ...
  socket.on('disconnect', () => {
    ...
    // Clean up UID → socket mapping
    if (socket.data.firebaseUid) {
      if (uidToSocket.get(socket.data.firebaseUid) === socket.id) {
        uidToSocket.delete(socket.data.firebaseUid);
      }
    }

    // Legacy ephemeral Orey ID cleanup
    if (socket.data.oreyId && !socket.data.firebaseUid) {
...
async function start() {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(`🔥 Firebase Auth: enabled (permanent Orey IDs)`);
      console.log(`⚧  Gender Match: 3s timer (server-side)`);
      console.log(`🌫️  Self-blur: enabled`);
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    });
  } catch (err) {
