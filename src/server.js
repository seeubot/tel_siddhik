...
<body>
  <div class="container"><div class="icon">🔷</div><h1>${SERVICE_NAME}</h1><p class="subtitle">Video Calling Platform</p><span class="status">🟢 Service Running</span></div>
</body>
</html>`);
  }
  next();
...
const verifySession = (req, res, next) => {
  ...
  if (session.expiresAt < Date.now()) { ADMIN_SESSIONS.delete(token); return res.status(401).json({ error: 'Session expired' }); }
  next();
};
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    activeAdminSessions: ADMIN_SESSIONS.size, notificationSubscriptions: notificationSubscriptions.size,
...
app.get('/generate-orey-id', (_req, res) => {
  ...
  res.json({ oreyId: displayId, hashId, expiresAt, format: 'OREY-XXXXX (5 characters)', validDuration: '24 hours' });
});
...
app.get('/api/notifications', (req, res) => {
  ...
  res.json({
    ...
    unread: filtered.filter(n => !n.isRead).length, serverTime: new Date().toISOString(),
...
app.get('/api/config', (req, res) => {
  const clientVersion = parseInt(req.query.version) || 0;
  res.json({
    features: { videoCall: true, hdVideo: clientVersion >= 2, groupCall: false, screenShare: clientVersion >= 3, reporting: true, safetyFeatures: true, chat: true, genderMatching: true },
...
app.post('/api/report', verifyApiKey, async (req, res) => {
  ...
  if (reportedDoc?.reportedBy?.includes(reporterDeviceId)) return res.status(400).json({ error: 'Already reported this user', alreadyReported: true });
  ...
  const shouldAutoBan = (isHighPriority && highPriorityCount >= HIGH_PRIORITY_BAN_THRESHOLD) || (!isHighPriority && currentCount >= AUTO_BAN_THRESHOLD);

  if (shouldAutoBan && appConfig.safety.autoBanEnabled) {
    const banDuration = isHighPriority ? 0 : DEFAULT_BAN_DURATION_HOURS;
    banInfo = { deviceId: reportedDeviceId, reason: isHighPriority ? `Auto-banned (HIGH PRIORITY): ${highPriorityCount} reports for "${reason}"` : `Auto-banned: ${currentCount} reports | Latest: ${reason}`, timestamp: new Date().toISOString(), expiresAt: banDuration > 0 ? new Date(Date.now() + banDuration * 3600000) : null, durationHours: banDuration > 0 ? banDuration : null, permanent: banDuration === 0, source: 'auto', isHighPriority, reportIds: [] };
...
app.post('/admin/login', (req, res) => {
  ...
  console.log(`🔐 Admin login. Session: ${sessionToken.substring(0, 16)}...`);
...
// ─── Admin endpoints (notifications, reports, bans, config) ───────────────────
...
app.get('/admin/notifications/stats', verifySession, (req, res) => {
  ...
  for (const n of notificationsCache) { if (!isNotifExpired(n)) stats.active++; if (!n.isRead) stats.unread++; stats.byType[n.type] = (stats.byType[n.type] || 0) + 1; stats.byPriority[n.priority] = (stats.byPriority[n.priority] || 0) + 1; }
  res.json(stats);
});
...
app.delete('/admin/notifications/:id', verifySession, async (req, res) => {
  ...
  notificationsCache.splice(idx, 1)[0];
...
app.get('/admin/reports', verifySession, async (req, res) => {
  ...
  res.json({ reports: allReports, total: allReports.length, pending: await Report.countDocuments({ status: 'pending' }), autoBanned: await Report.countDocuments({ status: 'auto_banned' }), highPriority: await Report.countDocuments({ isHighPriority: true }), activeBans: bannedDevicesCache.size });
});
...
app.post('/admin/reports/:reportId/action', verifySession, async (req, res) => {
  ...
  switch (action) {
    case 'ban': {
      ...
      report.status = 'banned'; break;
    }
    case 'dismiss': report.status = 'dismissed'; break;
    case 'warn': report.status = 'warned'; for (const [, s] of io.sockets.sockets) { if (s.data.deviceId === report.reportedDeviceId) s.emit('warning', { reason: report.reason, message: 'You have received a warning for violating community guidelines.' }); } break;
...
app.post('/admin/ban-device', verifySession, async (req, res) => {
  ...
  if (!deviceId) return res.status(400).json({ error: 'deviceId is required' });
  if (isDeviceBanned(deviceId)) return res.status(400).json({ error: 'Device already banned' });
...
app.delete('/admin/ban-device/:deviceId', verifySession, async (req, res) => {
  ...
  if (!found) return res.status(404).json({ error: 'Device not banned' });
...
app.get('/admin/stats', verifySession, async (req, res) => {
  res.json({
    activeConnections: io.engine.clientsCount, activeRooms: rooms.size, bannedDevices: bannedDevicesCache.size,
    pendingReports: await Report.countDocuments({ status: 'pending' }), totalNotifications: notificationsCache.length,
    activeAdminSessions: ADMIN_SESSIONS.size, uptime: process.uptime(), memoryMB: +(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2),
    safetySettings: appConfig.safety, dbState: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected',
...
app.get('/admin', (_req, res) => {
  ...
  res.status(200).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${SERVICE_NAME} Admin</title><style>body{font-family:system-ui;background:#0b0f17;color:#e2e8f0;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}.box{text-align:center;background:#1e293b;padding:2.5rem;border-radius:24px;border:1px solid #334155}h1{color:#3b82f6}</style></head><body><div class="box"><h1>🔷 ${SERVICE_NAME} Admin</h1><p>Admin panel not found. Place admin.html in public/</p></div></body></html>`);
});
...
app.get('*', (req, res) => {
  ...
  res.status(200).type('html').send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${SERVICE_NAME}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif;background:#0b0f17;color:#e2e8f0;display:flex;justify-content:center;align-items:center;min-height:100vh}.container{text-align:center;background:#1e293b;padding:3rem;border-radius:24px;border:1px solid #334155;max-width:500px;margin:20px}h1{color:#3b82f6;font-size:2.5rem;margin-bottom:0.5rem}.subtitle{color:#94a3b8;font-size:1.1rem;margin-bottom:2rem}.status{display:inline-block;background:#065f46;color:#6ee7b7;padding:0.5rem 1rem;border-radius:20px;font-size:0.9rem}.icon{font-size:3rem;margin-bottom:1rem}.links{margin-top:1.5rem;display:flex;gap:12px;justify-content:center;flex-wrap:wrap}.links a{color:#94a3b8;text-decoration:none;font-size:0.8rem;padding:6px 12px;border:1px solid #334155;border-radius:8px}.links a:hover{border-color:#3b82f6;color:#3b82f6}</style></head><body><div class="container"><div class="icon">🔷</div><h1>${SERVICE_NAME}</h1><p class="subtitle">Video Calling Platform</p><span class="status">🟢 Service Running</span><div class="links"><a href="/admin">Admin Panel</a><a href="/health">Health Check</a></div></div></body></html>`);
});
...
io.on('connection', (socket) => {
  ...
  socket.on('connect-by-orey-id', ({ targetOreyId }) => {
    ...
    if (!entry) { for (const [key, val] of oreyIds.entries()) { if (val.hashId === targetOreyId || key.startsWith(targetOreyId)) { entry = val; break; } } }
  ...
  // ─── 🆕 Gender-Aware Join Random (with 3-sec timer) ───────────────────────
  socket.on('join-random', () => {
    ...
    if (gender) {
      genderMatcher.startTimer(socket.id, gender);  // 🆕 Start 3-second timer
...
async function start() {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(`⚧  Gender Matchmaking: ACTIVE (3s timer)`);
